---
description: Using the FizzBuzz programming puzzle as a backdrop, this post shows how parallel states, guards, internal events work.
---
...
The most succinct way of expressing this dependency is using an _in_ guard, allowing the _digit_ to transition to on, if _fizz_ remains in _off_.

This 'in' guard is an example of an [internal event](glossary/internal-event.html){:.glossary}.  Internal events are events that are generated by the statechart itself as it transitions from state to state.  When the machine enters a state, it essentially sends an event saying "I'm now _in_ this state too" and that can trigger other transitions to fire.  This is what we want to happen here: We want the digit to transition only if it's also _in_ 'fizz.off':
...
There's one hitch though:  When the _increment_ event happens, both regions will enter their initial 'off' states.  This allows the guarded transition `in fizz.off` to happen immediately.  One way of solving this is to introduce a different initial state to 'fizz' so that it doesn't enter 'off' immediately; 'off' will signify a sort of _final_ state.  We'll call this state _pending_ and it will only have two automatic transitions out of it:  One to take it to 'on' if i%3==0, and another to take it to 'off' in all other cases:
